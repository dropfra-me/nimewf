# Low-level FFI mappings to libewf (1:1 names)

type
  libewf_handle_t* = object ## opaque
  libewf_error_t* = object ## opaque

{.push importc, cdecl.}

proc libewf_get_version*(): cstring

proc libewf_handle_initialize*(
  handle: ptr ptr libewf_handle_t, error: ptr ptr libewf_error_t
): cint

proc libewf_handle_free*(
  handle: ptr ptr libewf_handle_t, error: ptr ptr libewf_error_t
): cint

# Image option setters
proc libewf_handle_set_compression_values*(
  handle: ptr libewf_handle_t,
  compression_level: int8,
  compression_flags: uint8,
  error: ptr ptr libewf_error_t,
): cint

proc libewf_handle_set_maximum_segment_size*(
  handle: ptr libewf_handle_t,
  maximum_segment_size: culonglong,
  error: ptr ptr libewf_error_t,
): cint

proc libewf_handle_get_maximum_segment_size*(
  handle: ptr libewf_handle_t,
  maximum_segment_size: ptr culonglong,
  error: ptr ptr libewf_error_t,
): cint

# File segmentation controls
# Intentionally no segment_file_size API: prefer maximum_segment_size for portability.

proc libewf_handle_set_media_type*(
  handle: ptr libewf_handle_t, media_type: uint8, error: ptr ptr libewf_error_t
): cint

proc libewf_handle_get_media_type*(
  handle: ptr libewf_handle_t, media_type: ptr uint8, error: ptr ptr libewf_error_t
): cint

proc libewf_handle_set_media_flags*(
  handle: ptr libewf_handle_t, media_flags: uint8, error: ptr ptr libewf_error_t
): cint

proc libewf_handle_get_media_flags*(
  handle: ptr libewf_handle_t, media_flags: ptr uint8, error: ptr ptr libewf_error_t
): cint

proc libewf_handle_set_format*(
  handle: ptr libewf_handle_t, format: uint8, error: ptr ptr libewf_error_t
): cint

proc libewf_handle_get_format*(
  handle: ptr libewf_handle_t, format: ptr uint8, error: ptr ptr libewf_error_t
): cint

# Media size
proc libewf_handle_set_media_size*(
  handle: ptr libewf_handle_t, media_size: culonglong, error: ptr ptr libewf_error_t
): cint

proc libewf_handle_get_media_size*(
  handle: ptr libewf_handle_t, media_size: ptr culonglong, error: ptr ptr libewf_error_t
): cint

proc libewf_handle_set_bytes_per_sector*(
  handle: ptr libewf_handle_t, bytes_per_sector: uint32, error: ptr ptr libewf_error_t
): cint

proc libewf_handle_get_bytes_per_sector*(
  handle: ptr libewf_handle_t,
  bytes_per_sector: ptr uint32,
  error: ptr ptr libewf_error_t,
): cint

proc libewf_handle_set_sectors_per_chunk*(
  handle: ptr libewf_handle_t, sectors_per_chunk: uint32, error: ptr ptr libewf_error_t
): cint

proc libewf_handle_get_sectors_per_chunk*(
  handle: ptr libewf_handle_t,
  sectors_per_chunk: ptr uint32,
  error: ptr ptr libewf_error_t,
): cint

proc libewf_handle_get_compression_values*(
  handle: ptr libewf_handle_t,
  compression_level: ptr int8,
  compression_flags: ptr uint8,
  error: ptr ptr libewf_error_t,
): cint

# Optional/newer API in some builds; keep behind a define to avoid link issues on very old libewf
when defined(nimewfHeaderCodepage):
  proc libewf_handle_set_header_codepage*(
    handle: ptr libewf_handle_t, codepage: cstring, error: ptr ptr libewf_error_t
  ): cint

# Error helpers
proc libewf_error_free*(error: ptr ptr libewf_error_t): cint

proc libewf_error_sprint*(
  error: ptr libewf_error_t, string: cstring, size: csize_t
): cint

# Metadata (UTF-8) header value setters/getters
proc libewf_handle_set_utf8_header_value*(
  handle: ptr libewf_handle_t,
  identifier: ptr uint8,
  identifier_length: csize_t,
  utf8_string: ptr uint8,
  utf8_string_length: csize_t,
  error: ptr ptr libewf_error_t,
): cint

proc libewf_handle_get_utf8_header_value_size*(
  handle: ptr libewf_handle_t,
  identifier: ptr uint8,
  identifier_length: csize_t,
  utf8_string_size: ptr csize_t,
  error: ptr ptr libewf_error_t,
): cint

proc libewf_handle_get_utf8_header_value*(
  handle: ptr libewf_handle_t,
  identifier: ptr uint8,
  identifier_length: csize_t,
  utf8_string: ptr uint8,
  utf8_string_size: csize_t,
  error: ptr ptr libewf_error_t,
): cint

# Acquisition pipeline and open/close
proc libewf_get_access_flags_write*(): cint
proc libewf_get_access_flags_read*(): cint

proc libewf_handle_open*(
  handle: ptr libewf_handle_t,
  filenames: ptr cstring,
  number_of_filenames: cint,
  access_flags: cint,
  error: ptr ptr libewf_error_t,
): cint

proc libewf_handle_close*(
  handle: ptr libewf_handle_t, error: ptr ptr libewf_error_t
): cint

proc libewf_handle_write_buffer*(
  handle: ptr libewf_handle_t,
  buffer: pointer,
  buffer_size: csize_t,
  error: ptr ptr libewf_error_t,
): clong

proc libewf_handle_write_finalize*(
  handle: ptr libewf_handle_t, error: ptr ptr libewf_error_t
): clong

proc libewf_handle_read_buffer*(
  handle: ptr libewf_handle_t,
  buffer: pointer,
  buffer_size: csize_t,
  error: ptr ptr libewf_error_t,
): clong

proc libewf_handle_segment_files_corrupted*(
  handle: ptr libewf_handle_t, error: ptr ptr libewf_error_t
): cint

proc libewf_handle_get_number_of_checksum_errors*(
  handle: ptr libewf_handle_t,
  number_of_errors: ptr uint32,
  error: ptr ptr libewf_error_t,
): cint

proc libewf_handle_get_md5_hash*(
  handle: ptr libewf_handle_t,
  md5_hash: ptr uint8,
  size: csize_t,
  error: ptr ptr libewf_error_t,
): cint

proc libewf_handle_get_sha1_hash*(
  handle: ptr libewf_handle_t,
  sha1_hash: ptr uint8,
  size: csize_t,
  error: ptr ptr libewf_error_t,
): cint

proc libewf_handle_set_md5_hash*(
  handle: ptr libewf_handle_t,
  md5_hash: ptr uint8,
  size: csize_t,
  error: ptr ptr libewf_error_t,
): cint

proc libewf_handle_set_sha1_hash*(
  handle: ptr libewf_handle_t,
  sha1_hash: ptr uint8,
  size: csize_t,
  error: ptr ptr libewf_error_t,
): cint
